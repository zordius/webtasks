...
init = function (app) {
    ...
    app.page = appLocateRender(app, 'page');
    app.module = appLocateRender(app, 'module');
...
appLocateRender = function (app, type) {
    return function (name) {
        return function () {
            return locate(app, name, type).apply(this, arguments)
...
console.log('req page~~~');
    return locate(this, name, 'page')().transform(function (R) {
        return {
            layout: 'page_' + name,
            data: R
        };
...
console.log(arguments);
        return requestlocate(app, name, 'module').apply(app, arguments).transform(function (R) {
            return {
                layout: 'module_' + name,
                data: R
            };
...
render = function (R) {
    ...
    if (!R.layout) {
        error('internal error...render() without .layout');
        return subtask();
    }

    if (R.data === undefined) {
        error('rendering view: ' + R.layout + ' with undefined');
    }

    return subtask(function (cb) {
        hbsObj.render('view/' + R.layout + '.hbs', R.data).then(function (V) {
            cb(V);
        }, function (E) {
            cb();
            error(E);
